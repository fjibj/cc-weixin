/**
 * Pairing code + allowlist access control.
 * Pending pairings are persisted to disk so they can be confirmed from a different process.
 */
...
export function addPendingPairing(userId: string): string {
  ...
  pending[code] = { userId, expiresAt: now + 10 * 60 * 1000 }; // 10 min
  savePending(pending);
  return code;
}

/** Confirm a pairing code, returns userId if valid */
