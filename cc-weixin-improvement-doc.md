...
新增权限确认系统，实现敏感操作的微信审批机制。当 Claude 需要执行敏感工具（如 Bash、Edit 等）时，先发送权限请求到微信，用户通过回复 `yes CODE` 或 `no CODE` 来批准或拒绝。

### 9.2 实现架构

**方案选择**: 方案A'（基于定时任务架构）

与 claude-plugin-wechat 的通知机制不同，cc-weixin 采用定时任务驱动架构：
- 通过 `auto-process.ts` 轮询检测新消息
- 权限回复作为普通消息进入 queue.json
- `checkPermissionReplies()` 函数识别并处理权限回复
- 已处理的权限消息被过滤，不进入 Harness 流程

### 9.3 核心组件

#### 1. 权限请求数据结构 (`PermissionRequest`)
```typescript
interface PermissionRequest {
  requestId: string;      // 5位验证码（小写字母，不含l）
  chatId: string;         // 用户ID
...
#### 2. 验证码生成规则
- 长度：5位
- 字符集：小写字母 `a-km-z`（不含 `l`，避免与数字1混淆）
- 示例：`abc12`, `xyz89`

#### 3. 权限回复格式
- 批准: `yes CODE` 或 `y CODE`
- 拒绝: `no CODE` 或 `n CODE`
- 正则匹配: `/^\s*(y|yes|n|no)\s+([a-km-z]{5})\s*$/i`

### 9.4 使用流程

#### 1. 发送权限请求
```bash
bun run auto-process.ts test-permission <chatId> [contextToken]
```
...
#### 3. 系统处理
- 识别权限回复格式
- 更新 `pending_permissions.json` 中的状态
- 发送确认消息给用户
- 过滤该消息，不进入 Harness 流程

### 9.5 关键函数

| 函数 | 功能 |
|------|------|
| `generateRequestId()` | 生成5位验证码 |
| `sendPermissionRequest()` | 发送权限请求到微信 |
| `checkPermissionReplies()` | 检查并处理权限回复 |
| `isPermissionApproved()` | 查询指定权限状态 |
| `loadPermissions()` | 加载权限列表 |
| `savePermissions()` | 保存权限列表 |
...
用户发送权限回复 → queue.json
    ↓
auto-process.ts 轮询检测
    ↓
checkPermissionReplies() 识别格式
    ↓
更新 pending_permissions.json
    ↓
发送确认消息给用户
    ↓
过滤该消息（不进入 pending.json）
    ↓
Harness 流程只处理普通消息
```
...
权限请求保存在 `pending_permissions.json`：
```json
{
  "permissions": [
    {
      "requestId": "abc12",
      "chatId": "xxx@im.wechat",
      "contextToken": "...",
      "toolName": "Bash",
      "description": "执行系统命令测试",
      "timestamp": 1774837274440,
      "status": "approved"
    }
  ],
  "updatedAt": "2026-03-30T02:21:14.440Z"
}
```

### 9.8 安全考虑

1. **验证码随机性**: Math.random() 生成，5位组合约1100万种可能
2. **时效性**: 建议实现过期机制（如15分钟）
3. **唯一性**: 同一时间同 chatId 只能有一个 pending 权限请求
4. **状态不可篡改**: 一旦批准/拒绝，状态不可逆
...
3. **权限模板**: 预定义常用权限组合
4. **审计日志**: 记录所有权限操作历史
5. **与 Claude Code 深度集成**: 拦截工具调用自动触发权限请求
